            ...
            <div class="productIndexWrap">
                <div class="producIndex producIndexTable">
                    <div class="productWrapImg">
                        <c:if test="${not empty discount.images}">
                            <c:forEach var="img" items="${discount.images}">
                                <c:if test="${img.mainImg  == true}">
                                    <a href=""><img src="${img.pahtImg}" alt=""></a>
                                </c:if>
                            </c:forEach>
                        </c:if>
                        <c:if test="${empty discount.images}">
                            <a href=""><img src="resources/img/phone_index.jpg" alt="${discount.name}"></a>
                        </c:if>
                    </div>
                    <div class="productWrapTitle">
                        <h2 class="h2 dot-ellipsis dot-timer-update"><a href="good${discount.id}">${discount.name}</a></h2>
                    </div>
                    <div class="productWrapPrice">
                        <p>${discount.price }<span> грн</span></p>
                    </div>
                    <sec:authorize access="isAuthenticated()">
                        <div class="productWrapButtonBuy">
                            <a class="pure-button buttonGreen buttonBuy" id="g${discount.id}"><i class="fa fa-cart-plus fa-lg" aria-hidden="true"></i> Купити</a>
                        </div>
                    </sec:authorize>
                </div>
            </div>
